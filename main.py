...
for link in links:
    ...
    for clip in clips[:1]:
        ...
        youtube = Youtube(browser=driver)  # or firefox
        upload_result = youtube.upload(
            f"{clip_name}.mp4",
            f"{channel_name} - {clip_name}",
            f"Twitch Link = {clip_link}",
            ["Twitch Clip", "Twitch", "Clip", channel_name],
        )
